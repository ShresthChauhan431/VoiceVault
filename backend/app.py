...
def verify_voice():
    ...
    try:
            ...
            with timeout_context(AI_TIMEOUT_SECONDS):
                ...
                if session_id and session_id in embedding_store:
                    ...
                    if session_data['expires'] > time.time():
                        ...
                        has_session = True
                        logger.info(f'[VERIFY] Cosine similarity with session {session_id[:8]}... (after 0.90x): {cosine_score:.4f}')
                    else:
        ...
        if liveness_score < 0.10:
            logger.warning(f'[VERIFY] HARD GATE: Liveness {liveness_score:.4f} < 0.10 → DEEPFAKE')
            return jsonify({
                'status': 'deepfake_detected',
                'score': 0,
                'confidence_level': 'REJECTED',
        ...
        if has_session and cosine_score < 0.32:
            logger.warning(f'[VERIFY] HARD GATE: Cosine {cosine_score:.4f} < 0.32 → REJECTED')
            return jsonify({
                'status': 'rejected',
                'score': 0,
                'confidence_level': 'REJECTED',
